...
const createProductSupplier = async (req, res) => {
  ...
  try {
    ...
    await connectionPromise.query(CREATE_PRODUCT_SUPPLIER, [
      product_id,
      supplier_id,
    ]);
...
const updateProductSupplier = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connectionPromise.query(UPDATE_PRODUCT_SUPPLIER, [
      product_id,
      supplier_id,
      id,
    ]);
...
const deleteProductSupplier = (req, res) => {
  ...
  try {
    connection.query(DELETE_PRODUCT_SUPPLIER, id, (error, result) => {
      if (error) {
